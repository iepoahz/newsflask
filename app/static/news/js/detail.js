...
$(function(){
    ...
    //切换删除图片
     $(".comment_del").click(function () {
        $(this).toggleClass("has_comment_del")
    })
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.errno == '0') {
                ...
                // 拼接内容
                var comment_html = ''
                comment_html += '<div class="comment_list" id ="'+comment.id+'" >'
                comment_html += '<div class="person_pic fl">'
                comment_html += '<a href="'+ comment.user.id +'">'
                ...
                if (comment.user.id == resp.user_id){
                    comment_html+= '<a href="javascript:;" class="comment_del fr"  del_id="'+comment.id+'">删除</a>'
                }
...
if(sHandler.indexOf('reply_sub')>=0) {
    ...
    $.ajax({
        ...
        if(sHandler.indexOf('comment_del')>=0){
        var delid = $(this).attr("del_id")
        var params = {
            "del_comment_id": delid
        };
        $.ajax({
            url: "/news_comment/del",
            type: "post",
            contentType: "application/json",
            headers: {
                "X-CSRFToken": getCookie("csrf_token")
